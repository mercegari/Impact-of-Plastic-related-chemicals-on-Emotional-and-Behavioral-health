...
beta_line <- c("logit(p[i]) <- beta0") ## original binary specification
#beta_line <- c("p[i] <- beta0") ## adaptation for linear 
#beta_line <- c("p[i] <- r / (r + lambda[i])\nlog(lambda[i]) <- beta0") ## adaptation for negative binomial
...
# Combining Chunks
model_text <- c("model", "{", "for (i in 1:N)", "{", "y[i]  ~ dbern(p[i])", beta_line2, ## original binary specification
#model_text <- c("model", "{", "tau ~ dgamma(0.001, 0.001)", "for (i in 1:N)", "{", "y[i]  ~ dnorm(p[i], tau)", beta_line2, ## adaptation for linear
#model_text <- c("model", "{", "r ~ dunif(0, 100)", "for (i in 1:N)", "{", "y[i]  ~ dnegbin(p[i], r)", beta_line2, ## adaptation for negative binomial
